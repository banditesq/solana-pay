import { createTransferInstruction, getAccount, getAssociatedTokenAddress, getMint,createAssociatedTokenAccountInstruction, TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID } from '@solana/spl-token';
import {
    Commitment,
    Connection,
    LAMPORTS_PER_SOL,
...
export async function createTransfer(
    ...
    if (reference) {
        ...
        for (const pubkey of reference) {
            instruction.map(e=>e.keys.push({ pubkey, isWritable: false, isSigner: false }) );
        }
    }
    ...
    instruction.map((instruction: TransactionInstruction) => transaction.add(instruction));
...
async function createSPLTokenInstruction(
    ...
    try {
        ...
        if (recipientAccount.isFrozen) throw new CreateTransferError('recipient frozen');
         ...
         transaction.add(createAssociatedTokenAccountInstruction( sender,recipientATA,recipient,mint.address ))
    ...
    transaction.add(
        createTransferInstruction(senderATA,recipientATA,sender,BigInt(String(amount)),undefined,TOKEN_PROGRAM_ID)
